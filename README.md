## Отчёт по лабораторной работе №5 (трейсы + Grafana)

Краткий результат:

- Добавлен экспорт трейсов из приложения через OpenTelemetry (OTLP gRPC → `127.0.0.1:14317`).
- Поднят полный pipeline трейсов: `Application → Alloy (14317) → Tempo (4317) → Grafana`.
- Каждый HTTP-запрос оборачивается в span через `tracingMiddleware` в `cmd/server/main.go`.
- В Grafana подключён datasource `Tempo` (автоматически через provisioning).
- Проверены TraceQL-запросы в Grafana Explore.

Скриншот трейсов в Grafana:

![Trace в Grafana](imgs/trace.png)

![Trace в Grafana](imgs/trace2.png)

### Примеры TraceQL-запросов

```traceql
# Все трейсы сервиса
{ resource.service.name = "mini-task-tracker" }

# Трейсы с ошибками (status=error выставляется при 5xx)
{ resource.service.name = "mini-task-tracker" && status = error }

# Медленные трейсы
{ resource.service.name = "mini-task-tracker" && duration > 200ms }

# Трейсы по конкретной ручке (chi route pattern)
{ span.http.route = "/tasks/{id}" }
```
...
- Grafana Alloy — сбор логов (→ Loki) и трейсов (→ Tempo)
- OpenTelemetry SDK/OTLP — экспорт трейсов из приложения
...
└── configs/
    ...
    └── grafana/
        ├── provisioning/
        │   ├── datasources/ # авто-подключение Prometheus, Loki, Tempo
...
### Полный стек (приложение + Prometheus + Loki + Tempo + Alloy + Grafana)
...
Tempo недоступен в Homebrew — используется Docker fallback (`grafana/tempo:2.9.0`). **Docker Desktop должен быть запущен** перед `make run-all`.
...
Prometheus, Loki, Tempo, Alloy и Grafana запустятся в фоне, приложение - на переднем плане.
...
Сервисы также можно запустить по отдельности: `make run-prometheus`, `make run-loki`, `make run-tempo`, `make run-alloy`, `make run-grafana`.
...
**Tempo** → http://localhost:3200/ready — должен отвечать `ready` (через локальный бинарник или Docker fallback).

**Grafana** → http://localhost:3000 → дашборд **Mini Task Tracker** + Explore (datasource `Tempo`) показывают метрики, логи и трейсы.
